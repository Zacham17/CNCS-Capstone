...
def bytes_to_data_transfer_rate(bytes_value):
    return f"{round(bytes_value / (1024 * 1024), 2)} MB/s"
        ...
        if "_source" in item:
            ...
            if host not in monitored_hosts:
                monitored_hosts[host] = {
                    ...
                    "CPU Utilization": convert_to_percentage(source.get("system", {}).get("process", {}).get("cpu", {}).get("total", {}).get("pct", "")),
                    "Inbound Network Traffic": bytes_to_data_transfer_rate(source.get("system", {}).get("process", {}).get("io", {}).get("read_bytes", "")),
                    "Outbound Network Traffic": bytes_to_data_transfer_rate(source.get("system", {}).get("process", {}).get("io", {}).get("write_bytes", "")),
                    "Disk read": bytes_to_data_transfer_rate(source.get("system", {}).get("process", {}).get("io", {}).get("read_bytes", "")),
                    "Disk write": bytes_to_data_transfer_rate(source.get("system", {}).get("process", {}).get("io", {}).get("write_bytes", ""))
                }
